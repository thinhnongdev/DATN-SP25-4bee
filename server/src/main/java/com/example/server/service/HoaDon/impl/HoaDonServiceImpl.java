...
public class HoaDonServiceImpl implements IHoaDonService {
    ...
    public HoaDonResponse refundExcessPaymentToPending(String hoaDonId, BigDecimal soTien) {
        ...
        // 1. Kiểm tra số tiền điều chỉnh phải lớn hơn 0
        ...
        // 3. Kiểm tra số tiền thừa hiện có (đã tính cả phí vận chuyển)
        ...
        // 4. Kiểm tra số tiền điều chỉnh không vượt quá số tiền thừa
        if (soTien.compareTo(excessAmount) > 0) {
            throw new ValidationException(String.format(
                    "Số tiền điều chỉnh (%s) không được vượt quá số tiền thừa (%s)",
                    formatCurrency(soTien),
                    formatCurrency(excessAmount)));
        }

        // 5. Lấy các khoản thanh toán chờ xác nhận hoặc trả sau
        ...
        // 6. Biến để theo dõi số tiền còn lại cần điều chỉnh
        ...
        // 7. Xử lý phân bổ tiền hoàn vào các khoản thanh toán chờ/trả sau
        ...
        // 8. Tạo bản ghi "hoàn tiền" để theo dõi số tiền đã xử lý
        ...
        // 9. Tạo mô tả điều chỉnh chi tiết
        String adjustmentDescription = String.join("; ", adjustmentDetails);

        // 10. Lưu lịch sử điều chỉnh chung
        LichSuHoaDon lichSuHoaDon = new LichSuHoaDon();
        lichSuHoaDon.setId("LS" + UUID.randomUUID().toString().replace("-", "").substring(0, 8));
        lichSuHoaDon.setHoaDon(hoaDon);
        ...
        log.info(
                ...
                formatCurrency(excessAmount.subtract(actualAdjustedAmount)));
    ...
    public HoaDonResponse thanhToanPhuPhi(String hoaDonId, BigDecimal soTien, ThanhToanRequest thanhToanRequest) {
        ...
        // 4. Tính tổng tiền cần thanh toán = tổng tiền sản phẩm + phí vận chuyển
        ...
        BigDecimal tongTienCanThanhToan = tongTienSanPham.add(phiVanChuyen);

        // 5. Tính số tiền còn thiếu = tổng cần thanh toán - đã thanh toán thực tế
        BigDecimal soTienConThieu = tongTienCanThanhToan.subtract(tongTienThucTe);

        log.info(
                "Hóa đơn {}: Tổng tiền sản phẩm = {}, Phí vận chuyển = {}, Tổng cần thanh toán = {}, Đã thanh toán = {}, Đã hoàn = {}, Thực thanh toán = {}, Còn thiếu = {}, Sẽ thanh toán thêm = {}",
                hoaDon.getMaHoaDon(),
        ...
        // 6. Kiểm tra số tiền thanh toán có phù hợp không
        ...
        // 10. Lưu lịch sử thanh toán thêm
        LichSuHoaDon lichSuThanhToan = new LichSuHoaDon();
        lichSuThanhToan.setId("LS" + UUID.randomUUID().toString().replace("-", "").substring(0, 8));
        lichSuThanhToan.setHoaDon(hoaDon);
    ...
    public HoaDonResponse refundExcessPayment(
        ...
        // 2. Kiểm tra số tiền thừa thực tế (tính cả phí vận chuyển)
        BigDecimal excessAmount = calculateExcessPayment(hoaDonId);
        if (excessAmount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new ValidationException("Không có tiền thừa cần hoàn lại");
        }

        // 3. Kiểm tra số tiền hoàn lại không vượt quá số tiền thừa
        if (soTien.compareTo(excessAmount) > 0) {
            throw new ValidationException(String.format(
        ...
        log.info("Đã hoàn tiền thừa {} thành công cho hóa đơn {}", formatCurrency(soTien), hoaDonId);
    ...
    public BigDecimal calculateExcessPayment(String hoaDonId) {
        ...
        // 1. Tính tổng tiền đã thanh toán và hoàn tiền
        BigDecimal tongTienDaThanhToan = BigDecimal.ZERO;
        ...
        // 2. Số tiền thực tế đã thanh toán (đã trừ các khoản hoàn)
        BigDecimal soTienThucTeThanhToan = tongTienDaThanhToan.subtract(tongTienHoanTraLai);

        // 3. Tổng tiền cần thanh toán = Tổng tiền sản phẩm + Phí vận chuyển
        BigDecimal tongTienSanPham = hoaDon.getTongTien() != null ? hoaDon.getTongTien() : BigDecimal.ZERO;
        BigDecimal phiVanChuyen = hoaDon.getPhiVanChuyen() != null ? hoaDon.getPhiVanChuyen() : BigDecimal.ZERO;
        BigDecimal tongTienCanThanhToan = tongTienSanPham.add(phiVanChuyen);

        // 4. Xử lý đặc biệt với đơn hàng đã hủy
        ...
        // 5. Xử lý đơn hàng thông thường (không bị hủy)
        log.info(
                "Tính số tiền thừa - Hóa đơn {}: Tổng SP = {}, Phí vận chuyển = {}, Tổng cần thanh toán = {}, " +
                        "Đã thanh toán = {}, Đã hoàn = {}, Thực thanh toán = {}",
                hoaDonId,
                formatCurrency(tongTienSanPham),
                formatCurrency(phiVanChuyen),
                formatCurrency(tongTienCanThanhToan),
