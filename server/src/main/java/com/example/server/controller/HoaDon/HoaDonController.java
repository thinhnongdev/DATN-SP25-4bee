...
public class HoaDonController {
    ...
    public int layPhiVanChuyen(@RequestBody GHNTinhPhiRequest request) {
        // Kiểm tra xem có hoaDonId trong request không
        if (request.getHoaDonId() != null && !request.getHoaDonId().isEmpty()) {
            try {
                HoaDon hoaDon = hoaDonRepository.findById(request.getHoaDonId())
                        .orElse(null);

                // Nếu tìm thấy hóa đơn, sử dụng phiên bản mới của tinhPhiVanChuyen
                if (hoaDon != null) {
                    return ghnService.tinhPhiVanChuyen(request, hoaDon);
                }
            } catch (Exception e) {
                // Log lỗi nhưng vẫn tiếp tục với phương thức gốc
                System.err.println("Lỗi khi tìm hóa đơn: " + e.getMessage());
            }
        }

        // Sử dụng phương thức gốc nếu không có hoaDonId hoặc không tìm thấy hóa đơn
