...
import { getAllApi } from "../Api/KhachHangApi";
...
const KhachHang = ({ onAddClick, onDeleteClick, onEditClick, onViewClick }) => {
  ...
  const filteredNhanVien = khachhang.filter((item) => {
    const matchesSearch = item.ten_khach_hang?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
  ...
  const exportToExcel = () => {
      const worksheet = XLSX.utils.json_to_sheet(
        khachhang.map((item, index) => ({
          STT: index + 1,
          "Mã nhân viên": item.ma_khach_hang,
          "Tên nhân viên": item.ten_khach_hang,
          Email: item.email,
