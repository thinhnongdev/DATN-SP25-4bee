...
function InvoiceDetail() {
  ...
  const checkAndShowExcessPaymentNotification = () => {
    // Kiểm tra các điều kiện để không hiển thị thông báo không cần thiết
    if (
      !paymentHistory ||
      !invoice ||
      showExcessPaymentRefundDialog ||   // Đang hiển thị dialog hoàn tiền
      excessNotificationShown ||         // Đã hiển thị thông báo rồi
      invoice.trangThai === 6            // Đơn hàng đã hủy (sẽ có thông báo riêng)
    ) {
      return;
    }
  
    // Xác định số tiền thừa một cách chính xác
    const excessAmount = calculateExcessAmount();
    ...
    // Kiểm tra kỹ lưỡng trước khi hiển thị thông báo
    if (excessAmount > 1000) {
      // Kiểm tra nếu đã có hoàn tiền trước đó cho đơn hàng này
      const hasRefunds = paymentHistory.some(p => p.trangThai === 4);
      
      // Tính tổng số tiền hoàn trả
      const totalRefunded = paymentHistory
        .filter(p => p.trangThai === 4)
        .reduce((sum, p) => sum + (p.tongTien || p.soTien || 0), 0);
      
      // Chỉ hiển thị thông báo nếu số tiền thừa chưa được hoàn hết
      if (excessAmount > totalRefunded) {
        ...
        notification.warning({
          ...
          message: "Phát hiện thanh toán thừa",
          description: `Khách hàng đã thanh toán thừa ${formatCurrency(
            excessAmount - totalRefunded
          )}. Bạn nên xử lý hoàn tiền.`,
          btn: (
            <Button
              type="primary"
              onClick={() => {
                handleShowRefundDialog(excessAmount - totalRefunded);
  ...
  useEffect(() => {
    if (paymentHistory && paymentHistory.length > 0 && invoice?.tongTien) {
      // Đảm bảo có khoảng thời gian ngắn để dữ liệu được cập nhật trên state
      const timer = setTimeout(() => {
  ...
  const handleShowRefundDialog = (amount) => {
    ...
    // Cập nhật số tiền hoàn trả chính xác
    const refundAmount = amount || calculateExcessAmount();
    
    // Tính toán số tiền đã hoàn trả
    const totalRefunded = paymentHistory
      .filter(p => p.trangThai === 4)
      .reduce((sum, p) => sum + (p.tongTien || p.soTien || 0), 0);
    
    // Tính toán số tiền còn cần hoàn trả
    const remainingRefund = Math.max(0, refundAmount - totalRefunded);
    
    // Chỉ mở hộp thoại hoàn tiền nếu còn tiền cần hoàn
    if (remainingRefund > 0) {
      console.log("Số tiền còn cần hoàn:", remainingRefund);
      setExcessPaymentAmount(remainingRefund);
      setSelectedPaymentMethod(null);
      loadPaymentMethods();
      setShowExcessPaymentRefundDialog(true);
    } else {
      // Hiển thị thông báo nếu không còn tiền cần hoàn
      message.info("Không còn tiền thừa cần hoàn trả");
    }
  };
            ...
            >
              <div>
                <p style={{ marginBottom: '12px' }}>
                  Bạn muốn thay đổi trạng thái đơn hàng sang <strong>
                    {steps.find(step => step.status === nextStatus)?.label || 'Trạng thái mới'}
                  </strong>?
                </p>
                
                <Text>Vui lòng xác nhận bằng cách giữ nguyên hoặc sửa đổi ô bên dưới:</Text>
                <Input
                  value={confirmText}
                  ...
                  style={{ marginTop: '8px' }}
                  suffix={
                    confirmText.toLowerCase() === "đồng ý" ? (
