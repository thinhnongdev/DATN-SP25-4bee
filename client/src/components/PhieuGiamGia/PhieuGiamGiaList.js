...
const isFinalStatus = (trangThai) => {
  return trangThai === 2; // Trạng thái "Đã kết thúc"
};
...
const PhieuGiamGiaList = () => {
            ...
            Promise.all(
              changedItems.map(item =>
                updateTrangThaiPhieuGiamGia(item.id, item.trangThai)
                  .catch(error => {
                    message.error(`Lỗi cập nhật trạng thái cho ${item.maPhieuGiamGia}`);
                    return null;
                  })
              )
            ).then(() => {
              message.success("Đã đồng bộ trạng thái với server");
            });
          }
          setLastUpdated(new Date());
          setFilteredData(updatedData);
  ...
  const applyFilters = useCallback(() => {
    ...
    if (filters.loaiPhieuGiamGia) {
      result = result.filter(item => {
        if (filters.loaiPhieuGiamGia === "%") return item.loaiPhieuGiamGia === 1;
        if (filters.loaiPhieuGiamGia === "VND") return item.loaiPhieuGiamGia === 2;
        return true;
      });
    }

    if (filters.kieuPhieuGiamGia) {
      result = result.filter(item => item.kieuGiamGia === Number(filters.kieuPhieuGiamGia));
    }
  ...
  const exportToExcel = useCallback(() => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((item, index) => ({
        ...
        "Loại giảm giá": item.loaiPhieuGiamGia === 1 ? "%" : "VND",
        "Kiểu giảm giá": item.kieuGiamGia === 1 ? "Công khai" : "Cá nhân",
        "Giá trị giảm": item.loaiPhieuGiamGia === 1
          ? `${item.giaTriGiam}%`
          : `${item.giaTriGiam.toLocaleString()} VND`,
        "Số lượng": item.soLuong,
        "Đơn tối thiểu": item.giaTriToiThieu
          ? `${item.giaTriToiThieu.toLocaleString()} VND`
          : "Không",
        "Giảm tối đa": item.loaiPhieuGiamGia === 2 || item.soTienGiamToiDa == null
          ? "Không giới hạn"
          : `${item.soTienGiamToiDa.toLocaleString()} VND`,
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "soTienGiamToiDa",
      render: (value, record) =>
        record.loaiPhieuGiamGia === 2 || value == null
          ? "Không giới hạn"
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Tooltip title={isFinalStatus(record.trangThai) ? "Không thể chỉnh sửa phiếu đã kết thúc" : "Chỉnh sửa"}>
            <Button
              onClick={() => navigate(`/admin/update-phieu-giam-gia/${record.id}`)}
              icon={<EditOutlined />}
              disabled={isFinalStatus(record.trangThai)}
            />
          </Tooltip>
        </Space>
      ),
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={8}>
            <Select
              ...
              style={{ width: "100%" }}
              value={filters.loaiPhieuGiamGia}
              onChange={value => setFilters({ ...filters, loaiPhieuGiamGia: value })}
              allowClear
            >
          ...
          <Col xs={24} sm={12} md={12}>
            <Select
              placeholder="Chọn kiểu phiếu"
              style={{ width: "100%" }}
              value={filters.kieuPhieuGiamGia}
              onChange={value => setFilters({ ...filters, kieuPhieuGiamGia: value })}
              allowClear
            >
          ...
          <Col>
            <Space>
              <Button
                icon={<PlusOutlined />}
                type="primary"
                onClick={() => navigate("/admin/add-phieu-giam-gia")}
              >
                Thêm mới
              </Button>
