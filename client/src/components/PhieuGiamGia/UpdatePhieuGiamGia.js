...
import { Form, Input, Button, DatePicker, Select, notification, Row, Col, Table, Modal } from "antd";
...
const PhieuGiamGiaUpdate = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      try {
        ...
        const res = await axios.get(KHACH_HANG_URL, { headers: getAuthHeaders() });
  ...
  useEffect(() => {
    ...
    const fetchVoucherData = async () => {
      try {
        ...
        const data = res.data;
        if (!data) {
          notification.error({ message: "Không tìm thấy dữ liệu phiếu giảm giá!" });
          return;
        }
        ...
        const customerRes = await axios.get(`${API_URL}/${id}/khach-hang`, { headers: getAuthHeaders() });
        const appliedCustomers = customerRes.data || [];
        const activeCustomerIds = appliedCustomers
          .filter((c) => !c.trangThai) // Lọc khách hàng đang hoạt động
          .map((c) => c.id);
        setSelectedCustomers(activeCustomerIds);
      } catch (error) {
        notification.error({
          message: "Lỗi tải dữ liệu",
          description: error.response?.data?.message || "Không thể tải dữ liệu phiếu giảm giá!",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleDiscountTypeChange = (value) => {
    ...
    form.setFieldsValue({ soLuong: value === 2 ? selectedCustomers.length : form.getFieldValue("soLuong") });
  };
  ...
  const handleCustomerSelect = (selectedRowKeys) => {
    ...
    const handleAdd = async () => {
      try {
        await axios.put(`${API_URL}/${id}/add-customers`, newCustomers, { headers: getAuthHeaders() });
        ...
        notification.error({
          ...
          description: error.response?.data?.message || "Không thể thêm khách hàng!",
        });
      }
    };
    ...
    const handleRemove = async (khachHangId) => {
      try {
        await axios.patch(`${API_URL}/${id}/remove-customer/${khachHangId}`, null, { headers: getAuthHeaders() });
        setSelectedCustomers(selectedRowKeys);
    ...
    if (discountType === 2) {
      if (newCustomers.length > 0) {
        ...
        Modal.confirm({
          title: "Xác nhận xóa",
          content: "Bạn có chắc chắn muốn xóa khách hàng này khỏi phiếu giảm giá?",
          onOk: async () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/${id}`, requestData, { headers: getAuthHeaders() });
      notification.success({ message: "Cập nhật phiếu giảm giá thành công!" });
      navigate("/admin/phieu-giam-gia");
    } catch (error) {
      notification.error({
        message: "Lỗi cập nhật",
        description: error.response?.data?.message || "Không thể cập nhật phiếu giảm giá!",
      });
    } finally {
      setLoading(false);
    }
  ...
  const columns = [
    ...
    { title: "Email", dataIndex: "email", key: "email" },
    {
      ...
      key: "trangThai",
      render: (_, record) =>
        selectedCustomers.includes(record.id) ? <CheckCircleTwoTone twoToneColor="#52c41a" /> : null,
    },
  ];
