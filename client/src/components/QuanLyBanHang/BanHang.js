...
const BanHang = () => {
  ...
  // Địa chỉ
  // Cải thiện hàm handleAddressSelect để tự động tính phí vận chuyển và xử lý thông tin người nhận
  const handleAddressSelect = async (address) => {
    ...
    const messageKey = "updating-address";
    message.loading({
      content: "Đang cập nhật địa chỉ...",
      key: messageKey,
      duration: 0,
    });
    ...
    if (!activeKey) {
      message.warning({
        content: "Không tìm thấy hóa đơn để cập nhật địa chỉ.",
        key: messageKey,
        duration: 3,
      });
      return;
    ...
    // Thêm thông tin người nhận từ address vào payload nếu có
    const payload = {
      diaChiId: address.id,
      ...
      huyen: address.huyen,
      tinh: address.tinh,
      // Thêm thông tin người nhận và số điện thoại nếu có
      ...(address.tenNguoiNhan && { tenNguoiNhan: address.tenNguoiNhan }),
      ...(address.soDienThoai && { soDienThoai: address.soDienThoai }),
    };
    ...
    try {
      ...
      message.success({
        content: "Đã cập nhật địa chỉ giao hàng",
        key: messageKey,
        duration: 2,
      });

      // Tự động tính phí vận chuyển sau khi cập nhật địa chỉ
      message.loading({
        content: "Đang tính phí vận chuyển...",
        key: messageKey,
        duration: 0,
      });
      ...
      setTimeout(async () => {
        if (giaoHangRef.current) {
          try {
            ...
            if (isFreeShipping) {
              message.success({
                content: "Đơn hàng được miễn phí vận chuyển (≥ 2 triệu đồng)",
                key: messageKey,
                duration: 3,
              });
              ...
              message.success({
                content: `Đã tính phí vận chuyển: ${formatCurrency(
                  shippingFee
                )}`,
                key: messageKey,
                duration: 3,
              });
              handleShippingFeeUpdate(shippingFee);
            } else {
              message.info({
                content:
                  "Không thể tính phí vận chuyển, vui lòng kiểm tra lại địa chỉ",
                key: messageKey,
                duration: 3,
              });
            }
            ...
            message.error({
              content: "Không thể tính phí vận chuyển",
              key: messageKey,
              duration: 3,
            });
          }
      ...
      message.error({
        content: "Không thể cập nhật địa chỉ giao hàng, vui lòng thử lại.",
        key: messageKey,
        duration: 3,
      });
    }
  ...
  // Cải thiện hàm handleDeliveryMethodChange để tự động tính phí vận chuyển
  const handleDeliveryMethodChange = async (hoaDonId, method) => {
    ...
    try {
      // Hiển thị thông báo loading khi bắt đầu
      ...
      // Nếu chuyển sang giao hàng
      if (isDelivery) {
        // Thay đổi thông báo

        const currentTab = tabs.find((tab) => tab.key === hoaDonId);
        const hasCustomer = currentTab?.order?.khachHang;

        // Nếu không có khách hàng, hiện thông báo
        if (!hasCustomer) {
          message.warning({
            content: "Vui lòng chọn khách hàng để tính phí vận chuyển",
            key: messageKey,
            duration: 3,
          });
        }
        // Nếu có khách hàng, tự động chọn địa chỉ đầu tiên và tính phí vận chuyển
        else if (giaoHangRef.current) {
          // Đợi một chút để đảm bảo component GiaoHang đã được cập nhật với loại hóa đơn mới
          setTimeout(() => {
            // Tự động chọn địa chỉ đầu tiên
            const addressSelected = giaoHangRef.current.selectFirstAddress();

            if (addressSelected) {
              // Tính phí vận chuyển sau khi có địa chỉ
              setTimeout(async () => {
                try {
                  const shippingFee =
                    await giaoHangRef.current.calculateShippingFee();
                  ...
                  if (isFreeShipping) {
                    message.success({
                      content:
                        "Đơn hàng được miễn phí vận chuyển (≥ 2 triệu đồng)",
                      key: messageKey,
                      duration: 3,
                    });
                    handleShippingFeeUpdate(0);
              ...
              message.info({
                content:
                  "Khách hàng chưa có địa chỉ, vui lòng thêm địa chỉ để tính phí vận chuyển",
                key: messageKey,
                duration: 3,
              });
            }
        ...
        message.success({
          content: "Đã chuyển sang hình thức tại quầy",
          key: messageKey,
          duration: 2,
        });
      }
      ...
      message.error({
        content: "Không thể thay đổi loại hóa đơn",
        key: "updating-delivery-method",
        duration: 3,
      });
    }
  };
