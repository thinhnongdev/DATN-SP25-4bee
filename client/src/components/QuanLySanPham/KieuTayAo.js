...
const KieuTayAo = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      Modal.confirm({
        title: isEditing ? 'Xác nhận sửa kiểu tay áo?' : 'Xác nhận thêm kiểu tay áo?',
        content: `Bạn có chắc chắn muốn ${isEditing ? 'sửa' : 'thêm'} kiểu tay áo "${
          values.tenKieuTayAo
        }" không?`,
        okText: 'Xác nhận',
        cancelText: 'Hủy',
        onOk: async () => {
          try {
            if (isEditing) {
              // Cập nhật
              await axios.patch(
                `https://datn-sp25-4bee.onrender.com/api/admin/kieutayao/${editingRecord.id}`,
                values,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
              ...
              const response = await axios.post(
                'https://datn-sp25-4bee.onrender.com/api/admin/addkieutayao',
                values,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      ...
      >
        <Breadcrumb.Item>
        <span style={{fontSize:'20px'}}>Kiểu tay áo</span>
       </Breadcrumb.Item>
      </Breadcrumb>
      <div
        style={{
